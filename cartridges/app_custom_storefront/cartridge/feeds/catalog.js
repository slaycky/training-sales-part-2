...
/**
 * Returns variation string value
 * @param {String} variationAttributeID
 * @param {dw.catalog.Product} product
 * @returns {String} variationStringValue
 */
function getVariationAttributeStringValue(variationAttributeID, product) {
    var variationStringValue = '';
    ...
    if (!empty(variationAttribute)) {
        variationValue = product.variationModel.getVariationValue(product, variationAttribute);
        if(!empty(variationValue)) {
            variationStringValue = variationValue.displayValue;
        }
    } else {
        for each(var varAttr in product.variationModel.getProductVariationAttributes()) {
            if (varAttr.getAttributeID().toLowerCase() == variationAttributeID) {
                variationValue = product.variationModel.getVariationValue(product, varAttr)
                if(!empty(variationValue)) {
                    variationStringValue = variationValue.displayValue;
                }
                break;
    ...
    return variationStringValue;
}

/**
 * Returns size variation value
 * @param cfg
 * @param data
 * @returns {String}
 */
function color(cfg, data) {
    var color = '';

    if (cfg.hasOwnProperty('variationID')) {
        color = getVariationAttributeStringValue(cfg.variationID, data.Product);
    }
 ...
 * @returns {String}
 */
function size(cfg, data) {
    var size;

    if (cfg.hasOwnProperty('variationID')) {
        size = getVariationAttributeStringValue(cfg.variationID, data.Product);
    }
